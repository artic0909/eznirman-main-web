    ...
    <section class="transactions-section">
      ...
      <div class="ledger-list" id="allLedgerList">
        @forelse($transactions as $tx)
          <div class="ledger-item tx-row" data-note="{{ strtolower($tx->note) }}" onclick="openReceiptModal('{{ $tx->note }}', 'EZ-TX-{{ $tx->id }}', '{{ $tx->type === 'credit' ? '+' : '-' }}₹{{ number_format($tx->amount, 2) }}', 'Success', '{{ $tx->date->format('d M Y') }} · {{ $tx->created_at->format('h:i:s A') }}', '{{ $tx->accountcode ? $tx->accountcode->name : 'N/A' }}', '{{ $tx->note }}', '₹{{ number_format($tx->balance_after, 2) }}')">
            <div class="ledger-left">
              ...
              <div class="ledger-details">
                ...
                <span class="ledger-sub">
                  {{ $tx->accountcode ? $tx->accountcode->name : 'General Ledger' }} · {{ $tx->date->format('d M Y') }} · {{ $tx->created_at->format('h:i:s A') }}
                </span>
