  ...
  <div class="col-left" style="grid-column: 1 / -1;">
    ...
    <section class="monthly-summary-card" style="grid-template-columns: 1fr 1fr; margin-bottom: 8px;">
      ...
      <div class="summary-stat-box" style="padding-left: 10px;">
        <span class="summary-stat-lbl">Total Ledger Volume</span>
        <span class="summary-stat-val highlight" style="color: var(--white); font-size: 24px;">{{ $transactions->total() }} entries</span>
      </div>
    </section>
    ...
    <section class="transactions-section">
      ...
      <div class="ledger-list" id="allLedgerList">
        @forelse($transactions as $tx)
          <div class="ledger-item tx-row" data-note="{{ strtolower($tx->note) }}" onclick="openReceiptModal('{{ $tx->note }}', 'EZ-TX-{{ $tx->id }}', '{{ $tx->type === 'credit' ? '+' : '-' }}₹{{ number_format($tx->amount, 2) }}', 'Success', '{{ $tx->created_at->format('d M Y') }} · {{ $tx->created_at->format('h:i:s A') }}', '{{ $tx->accountcode ? $tx->accountcode->name : 'N/A' }}', '{{ $tx->note }}', '₹{{ number_format($tx->balance_after, 2) }}')">
            <div class="ledger-left">
              ...
              <div class="ledger-details">
                ...
                <span class="ledger-sub">
                  {{ $tx->accountcode ? $tx->accountcode->name : 'General Ledger' }} · {{ $tx->created_at->format('d M Y') }} · {{ $tx->created_at->format('h:i:s A') }}
                </span>
