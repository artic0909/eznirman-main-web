...
@section('content')
  ...
  <div class="col-left" style="grid-column: 1 / -1;">
    ...
    <section class="transactions-section">
      ...
      <div class="ledger-list" id="creditLedgerList">
        @forelse($credits as $tx)
          <div class="ledger-item credit-row" data-note="{{ strtolower($tx->note) }}" onclick="openReceiptModal('{{ $tx->note }}', 'EZ-TX-{{ $tx->id }}', '+₹{{ number_format($tx->amount, 2) }}', 'Success', '{{ $tx->date->format('d M Y · h:i:s A') }}', '{{ $tx->accountcode ? $tx->accountcode->name : 'N/A' }}', '{{ $tx->note }}', '₹{{ number_format($tx->balance_after, 2) }}')">
            <div class="ledger-left">
              <div class="ledger-icon-wrap credit">✓</div>
              <div class="ledger-details">
                <span class="ledger-desc">{{ $tx->note }}</span>
                <span class="ledger-sub">
                  {{ $tx->accountcode ? $tx->accountcode->name : 'Allocation Release' }} · {{ $tx->date->format('d M Y · h:i:s A') }}
                </span>
        ...
        @empty
          <div style="text-align: center; padding: 60px; color: var(--text-muted); font-size: 13px;">
            No credit allocations registered.
          </div>
        @endforelse
      </div>
