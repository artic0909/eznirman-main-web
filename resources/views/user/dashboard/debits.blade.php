    ...
    <section class="transactions-section">
      ...
      <div class="ledger-list" id="debitLedgerList">
        @forelse($debits as $tx)
          <div class="ledger-item debit-row" data-note="{{ strtolower($tx->note) }}" onclick="openReceiptModal('{{ $tx->note }}', 'EZ-TX-{{ $tx->id }}', '-₹{{ number_format($tx->amount, 2) }}', 'Success', '{{ $tx->created_at->format('d M Y') }} · {{ $tx->created_at->format('h:i:s A') }}', '{{ $tx->accountcode ? $tx->accountcode->name : 'N/A' }}', '{{ $tx->note }}', '₹{{ number_format($tx->balance_after, 2) }}')">
            <div class="ledger-left">
              ...
              <div class="ledger-details">
                ...
                <span class="ledger-sub">
                  ...
                  @endif
                  {{ $tx->accountcode ? $tx->accountcode->name : 'General Disbursal' }} · {{ $tx->created_at->format('d M Y') }} · {{ $tx->created_at->format('h:i:s A') }}
                </span>
      ...
      <div style="margin-top: 20px; display: flex; justify-content: center;">
        {{ $debits->links('vendor.pagination.custom') }}
      </div>
    </section>
