    ...
    <section class="transactions-section">
      ...
      <div class="ledger-list" id="debitLedgerList">
        @forelse($debits as $tx)
          <div class="ledger-item debit-row" data-note="{{ strtolower($tx->note) }}" onclick="openReceiptModal('{{ $tx->note }}', 'EZ-TX-{{ $tx->id }}', '-₹{{ number_format($tx->amount, 2) }}', 'Success', '{{ $tx->date->format('d M Y · h:i:s A') }}', '{{ $tx->accountcode ? $tx->accountcode->name : 'N/A' }}', '{{ $tx->note }}', '₹{{ number_format($tx->balance_after, 2) }}')">
            <div class="ledger-left">
              <div class="ledger-icon-wrap debit">↑</div>
              <div class="ledger-details">
                <span class="ledger-desc">{{ $tx->note }}</span>
                <span class="ledger-sub">
                  {{ $tx->accountcode ? $tx->accountcode->name : 'General Disbursal' }} · {{ $tx->date->format('d M Y · h:i:s A') }}
                </span>
